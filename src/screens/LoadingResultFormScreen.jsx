...
function LoadingResultForm({ auth, masters, schedule, result, details, onLocalSave, onOpenDeliveryResult, onClose }) {
  ...
  const [rows, setRows] = useState(() => {
    ...
    return [{ id: null, 池場ID: schedule['池場ID'], バケット容量: '', 実績数量kg: schedule['予定数量kg'] || '' }];
  });
  ...
  function addRow() {
    ...
    setRows((rs) => [...rs, { id: null, 池場ID: schedule['池場ID'], バケット容量: '', 実績数量kg: '' }]);
  }
  ...
  async function handleSave() {
    ...
    const detailData = rows.map((r) => {
      const d = { 池場ID: r['池場ID'], バケット容量: r['バケット容量'], 実績数量kg: r['実績数量kg'] };
    ...
    <div>
      ...
      {rows.map((r, i) => (
        ...
        >
          ...
          <Field label="池場">
            <select value={r['池場ID']} onChange={(e) => setRow(i, '池場ID', e.target.value)} style={inputStyle}>
              {(masters?.池場 || []).map((p) => (
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <Field label="実績数量（kg）" style={{ flex: 1 }}>
              <input
                type="number"
                value={r['実績数量kg']}
            ...
            <Field label="バケット容量" style={{ flex: 1 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                {['250', '500'].map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => setRow(i, 'バケット容量', r['バケット容量'] === v ? '' : v)}
                    style={{
                      flex: 1,
                      minHeight: 42,
                      borderRadius: 8,
                      fontSize: 15,
                      border: '1px solid var(--c-border-2)',
                      background: r['バケット容量'] === v ? 'var(--c-accent)' : 'transparent',
                      color: r['バケット容量'] === v ? '#03202e' : 'var(--c-text)',
                      cursor: 'pointer',
                    }}
                  >
                    {v}kg
